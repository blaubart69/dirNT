...
template<typename C>
DWORD NtEnum<C>::_internal_enum_recurse(const HANDLE hDir, const size_t depth)
{
	if (_findBufs.size() < depth + 1)
	{
		BYTE* findBuf = (BYTE*)HeapAlloc(GetProcessHeap(), 0, FIND_BUF_LENGTH);
		_findBufs.push_back(findBuf);
	}
	...
	for (;;)
	{
		...
		else
		{
			walkDirectoryInformation(
				...
				, [&hDir, this, depth](nt::FILE_DIRECTORY_INFORMATION* info)
			{
				_pfnOnEntry(_dir, _rootDirLen, info);

				HANDLE hSubDir;
				nt::NTSTATUS statSubdir;
				if ((info->FileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0)
				{
				}
				else if ((statSubdir = openSubDir(&hSubDir, hDir, info->FileName, info->FileNameLength)) != STATUS_SUCCESS)
				{
					// toDo: error
				}
				else
				{
					const size_t lastLength = _dir.length();
					_dir.push_back(L'\\');
					_dir.append(info->FileName, info->FileNameLength / sizeof(WCHAR));
					_internal_enum_recurse(hSubDir, depth + 1);
					_dir.resize(lastLength);
				}
			});
		}
	}
